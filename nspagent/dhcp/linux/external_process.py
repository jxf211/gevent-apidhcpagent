...
class ProcessMonitor(object):
    ...
    def _spawn_checking_thread(self):
        ...
        gevent.spawn(self._periodic_checking_thread)
    ...
    def _periodic_checking_thread(self):
        while self._monitor_processes:
            gevent.sleep(self._config.AGENT.check_child_processes_interval)
            gevent.spawn(self._check_child_processes)
    ...
    def _respawn_action(self, service_id):
        LOG.error(("respawning %(service)s for uuid %(uuid)s"),
                  {'service': service_id.service,
                   'uuid': service_id.uuid})
